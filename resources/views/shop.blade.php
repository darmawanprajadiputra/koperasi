...
<div class="px-8 py-8 max-w-7xl mx-auto">
  <header class="mb-6">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div>
        <h1 class="font-manrope text-4xl font-extrabold text-primary uppercase mb-2">
          Shop
        </h1>
      </div>
    </div>
  ...
  <div id="categoryFilter" class="flex flex-wrap gap-2 mb-6">
    <span class="category-badge px-6 py-2 bg-tertiary text-on-tertiary rounded-full text-sm font-medium cursor-pointer transition-colors" data-category="all">
      Semua Produk
    </span>
  </div>
  ...
  <div id="emptyState" class="hidden text-center py-12">
    ...
    <p class="text-on-surface-variant text-sm mt-1">Tambahkan produk di halaman Gudang untuk ditampilkan di toko.</p>
  </div>
...
<div id="floatingCart" class="fixed bottom-6 right-6 z-50 hidden">
  <button id="checkoutBtn"
    ...
    <div class="relative">
      ...
      <span id="cartBadge" class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">0</span>
    </div>
...
<script>
  // Attach checkout button AFTER shop.js has loaded
    ...
    if (btn) {
      btn.addEventListener('click', function () {
        if (typeof window.goToCheckout === 'function') {
          ...
          // Fallback: jika shop.js belum siap, simpan cart lalu redirect
